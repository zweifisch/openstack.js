...
exports.defres = function (service, path, singular, opts) {
    ...
    var run = function (token) {
        ...
        return result.then(function({header, body}) {
            ${(action === "list" ? plural : _singular) ? `body = body["${action === "list" ? plural : _singular}"]` : ''}
            return (options && options.requestId) ? [header['x-openstack-request-id'] || header['x-compute-request-id'], body] : body
        })
    }
    if ("function" === typeof token.then) {
